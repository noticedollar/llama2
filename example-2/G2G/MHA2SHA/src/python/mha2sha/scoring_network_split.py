...
class ScoringNetworkSpliter:
    ...
    def split_batch_matmul(self):
        if self.use_conv:
            model = self._split_batch_matmul_to_conv()
        else:
            model = self._split_batch_matmul()
...
if __name__ == "__main__":
    ...
    spliter = ScoringNetworkSpliter(model, use_conv)
    split_model = spliter.split_batch_matmul()
    ou.save_model(split_model, onnx_output_filename)
    print(f"Model saved at {onnx_output_filename}")
    ...
    _, converted_model_outputs = ou.run_model_on_ort(
        onnx_output_filename, np_inputs, output_names
    )
