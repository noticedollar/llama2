        ...
        for key, value in batch.items():  # for each node in the pkl file
            if self.pickle_type == "npy":
                ...
                if len(value) == 2 and isinstance(value[0], (tuple, list)) and isinstance(value[0][0], (tuple, list)):
                    yield from [(f'past_key_{i}_out', v[0]) for i, v in enumerate(value[0])]
                    yield from [(f'past_value_{i}_out', v[1]) for i, v in enumerate(value[0])]
...
def get_split_tensors(onnxfile, onnxmodel=None, include_first_input=True, include_all_adds=False):
    ...
    def get_chain_of_adds(nodes):
        ...
        all_adds = [node for node in nodes if node.op_type in ("Add", "Cast")]
...
def split_onnx_by_names(onnxfile, modelname, pickle_filedir, *list_of_output_tensors, output_dir='.', onnxmodel=None, encoding_file=None, embed_ssd_params_file=None, using_qairt_workflow=False, dump_embedding_table_data=False):
...
def split_onnx(onnxfile, modelname, pickle_filedir, num_splits, output_dir='./', split_embedding=False, split_lmhead=False, encoding_file=None, embed_ssd_params_file=None, using_qairt_workflow=False, lora_importer_config=None):
    ...
    num_layers_per_split = (num_layers + remaining_splits - 1) // remaining_splits
    ...
    if split_lmhead:
        outputs = [output_tensor_list[num_layers-1]]
        for layer in range(num_layers-num_layers_per_split, num_layers):
            outputs += past_key_values[layer]
            outputs += past_anchor_values[layer]
        names_to_split.append(','.join(outputs))
